...
legs â€” so I built an agent that does the morning routine *for* me, before I
wake up.

**DayBreak** is a personal AI agent that runs completely unattended. Every day
at 6:00 AM Pakistan time, Amazon EventBridge Scheduler wakes it up â€” no button,
...
**[SCREENSHOT 1: the brief email arriving in my inbox â€” upload `docs/screenshots/1-email-header-2025pkt.png`]**

*(The complete email â€” headlines and all â€” is in the repo under `docs/screenshots/`.)*

And because the whole point of the challenge is that it happened without me,
each email's footer states exactly how the run was triggered â€” the
...
learning the platform as we went. The guiding principle was simplicity â€”
...
- **Keyless data sources.** Weather comes from the free Open-Meteo API and
  headlines from public RSS feeds (BBC World, Ars Technica) â€” zero API keys,
...
- **EventBridge Scheduler instead of classic cron rules**, because it is
  ...
  means.
- **SNS for delivery instead of SES** â€” one topic, one email subscription,
...
1. **IAM is eventually consistent.** My first `lambda create-function` failed
   with "the role cannot be assumed by Lambda" â€” a freshly created IAM role
...
3. **Honest evidence needed engineering.** My first test email proudly claimed
   it was "triggered by EventBridge Scheduler" â€” but I had invoked it by hand.
   The fix: the schedule injects its own context attributes
   (`<aws.scheduler.schedule-arn>`, `<aws.scheduler.scheduled-time>`) into the
   event payload, and the agent describes its trigger truthfully â€” manual test
   runs are labeled as manual test runs.

## AWS Services Used / Architecture Overview

```
EventBridge Scheduler â”€â”€(6:00 AM PKT daily, no human)â”€â”€> AWS Lambda (Python 3.13)
                                                            â”‚
                             ...
                             (weather, free)          (headlines)        (Nova Micro writes
                                    ...
                                    â””â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”¬â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜                 â”‚
                                                â–¼                             â”‚
                                          Amazon SNS  <â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
                                                â”‚
                                                â–¼
                                          ðŸ“§ my inbox (before I wake up)
```

**[SCREENSHOT 2: EventBridge schedule in the console â€” cron + Asia/Karachi timezone visible]**
...
| **Amazon Bedrock â€” Nova Micro** | Writes the brief from raw weather + headlines | ~$0.0001 per brief |
...
Cost was a hard requirement for me â€” I'm a student and wanted this at $0. The
...
- **Model access got simpler.** Bedrock's "Model access" page was retired â€”
...
- **IAM eventual consistency, exit codes for native CLIs, and `file://` for
  complex parameters** â€” three deployment lessons I'll reuse forever.
...
**[SCREENSHOT 4: the email footer showing the schedule name â€” proof it ran unattended]**
